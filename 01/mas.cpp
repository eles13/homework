...
int binsearch(vector<int> Data,const int size,const int left,const int right,const int node)
{
  if (left==right) return left;
  if (node<Data[0]) return 0;
  int k=int((right+left)/2);
  if (Data[k]<node)
  {
    if (k+1<size)
    {
    ...
    return binsearch(Data,size,k,right,node);
  ...
  else if (Data[k]>node)
  {
    ...
    return binsearch(Data,size,left,k,node);
  }
  else
  return k;
}
