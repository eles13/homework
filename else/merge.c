...
static void merge(int *m1, int *m2, int size, int pos, int *res, int part,
                  const int oddsize) {
  if (!part) {
    size++;
    pos++;
  ...
  //printf("size %d pos %d thr %d starting from %d %d\n", size, pos, omp_get_thread_num(), m1[0], m2[0]);
  int a = 0, b = 0, i = 0;
  for (; i < size + pos; i++) {
...
//     { out(m, n); }
    ...
    for (int pow = 2; pow <= numthreads; pow *= 2) {
      ...
      if (thrnum < numthreads / pow) {
        ...
        //	{
        //printf("num %d\n", omp_get_thread_num());
        if ((thrnum == numthreads / pow - 1) && (n%2==1)) {
          oddsize = 1;
        }
        int j = 0;
        int *buf = calloc(size * pow / 2, sizeof(int));
        assert(buf);
        for (int i = size * pow * thrnum;
             i < size * pow * thrnum + size * pow / 2; i++) {
          buf[j] = m[i];
          j++;
        }
        ...
        assert(buf2);
        j = 0;
        for (int i = size * thrnum * pow + size * pow / 2;
             i < size * thrnum * pow + size * pow + oddsize; i++) {
          buf2[j] = m[i];
          // if (thrnum == 0) {
...
#pragma omp task firstprivate(buf, buf2, defpos, pow, size, oddsize)
        ...
        // pow/2 + 1], buf2[pos+1], pow * (size - size/2) - 1, size * pow -
        // pos-2);
#pragma omp task firstprivate(buf, buf2, defpos, pow, size, oddsize)
        {
          // printf("start 1 %d start 2 %d size 1 %d size 2 %d start
          // %d\n",defpos + 1, pos+1, pow * (size - size/2) / 2 - 1, size * pow /
          // 2 - pos - 1, size * pow * thrnum  + size*pow/4 + pos + 2);
          if (size * pow / 4 % 2 == 1)
          {
            oddchunk = 1;
          }
          merge(&buf[defpos + 1], &buf2[pos + 1],
                pow * (size - size / 2) / 2 - 1 - oddchunk, size * pow / 2 - pos - 1,
