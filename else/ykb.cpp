...
void choosing(const Matrix& A, size_t& i,size_t& j, const int iter, const size_t size, const int strat)
{
...
int ykb(Matrix& A,Matrix& U, const double eps, vector<double>& values, const size_t size)
{
  ...
  while (A.sod() > eps)
  {
    choosing(A, i, j, iters, size, strat);
    cos  = Cos(A[i][j]*(-2),A[i][i] - A[j][j]);
    sin = Sin(A[i][j]*(-2),A[i][i] - A[j][j]);
    ii = pow(cos,2)*A[i][i] + pow(sin,2)*A[j][j] - 2*cos*sin*A[i][j];
    jj =  pow(cos,2)*A[j][j] + pow(sin,2)*A[i][i] + 2*cos*sin*A[i][j];
    for(size_t k = 0; k < size; k++)
    {
...
int main(int argc, char** argv)
{
  ...
  double tme = clock();
  int iters = ykb(A, U, 0.0001, values, size);
  tme = clock() - tme;
  cout<<"Values:"<<endl;
  for(size_t i = 0; i < size; i++)
    cout<<values[i]<<' ';
